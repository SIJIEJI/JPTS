...
class Cost2100DataLoader(object):
    ...
    def __init__(self, root, batch_size, num_workers, pin_memory, scenario):
        ...
        padd = torch.nn.ZeroPad2d((0,1,0,1)) 
        data_train = padd(data_train) 
        
        suff = suffler(data_train) 
        aux_data_train = suff.suffle()
...
class suffler():
    ...
    def suffle(self):
        ...
        idx = []
        idx.extend([[0,11,0,11], [0,11,11,22], [0,11,22,33],
                    [11,22,0,11], [11,22,11,22], [11,22,22,33],
                    [22,33,0,11], [22,33,11,22], [22,33,22,33]])
        s = torch.zeros((100000,2,33,33))
        for i in range(0, self.origin.size(0)):
            seq = self.gen_sequence(len(idx))
            temp = torch.zeros((9,2,11,11))
            for ii in range(0, len(idx)):
                temp[ii,:,:,:] = self.origin[i,:,idx[ii][0]:idx[ii][1],idx[ii][2]:idx[ii][3]]
            tar = torch.zeros((9,2,11,11))
