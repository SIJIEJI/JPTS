...
class Trainer:
    ...
    def train(self, train_loader):
        ...
        with torch.enable_grad():
            return self._iteration_train(train_loader)
    ...
    def val(self, val_loader):
        ...
        self.model.eval()
        with torch.no_grad():
            return self._iteration_val(val_loader)
    ...
    def _iteration_train(self, data_loader):
        iter_loss = AverageMeter('Iter loss')
        iter_time = AverageMeter('Iter time')
        time_tmp = time.time()
        #

        for batch_idx, (sparse_gt, label ) in enumerate(data_loader):
            
            sparse_gt = sparse_gt.to(self.device)
            label = label.to(self.device) #[200,9]
            
            gt_ori = sparse_gt[:,0:2,:,:]
            gt_jsaw = sparse_gt[:,2:4,:,:]
            
            sparse_pred1 = self.model(gt_ori)
            _,sparse_pred2 = self.model.clencoder(gt_jsaw) # [200,9,9]
            loss1 = self.criterion(sparse_pred1, gt_ori)
            loss2 = []
            for i in range(label.size(0)):
                temp = torch.nn.functional.cross_entropy(sparse_pred2[i],label[i].type(torch.LongTensor).cuda(),reduction='sum')
                loss2.append(temp.item())
            loss2 = np.array(loss2)
            loss2 =loss2.mean()
            
            loss = 0.5*loss1 + 0.5*loss2   #later make it adpative
    ...
    def _iteration_val(self, data_loader):
        iter_loss = AverageMeter('Iter loss')
        iter_time = AverageMeter('Iter time')
        time_tmp = time.time()
...
class Tester:
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, (sparse_gt, raw_gt) in enumerate(data_loader):
            ...
            sparse_pred = self.model(sparse_gt)
            loss = self.criterion(sparse_pred, sparse_gt)
            nmse = evaluator(sparse_pred, sparse_gt, raw_gt)
        ...
        logger.info(f'=> Test rho:{iter_nmse.avg:.3e}  NMSE: {iter_nmse.avg:.3e}\n')

        return iter_loss.avg, 0, iter_nmse.avg
