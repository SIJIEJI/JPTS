...
class CRNet(nn.Module):
    def __init__(self, reduction=4):
        ...
        total_size, in_channel, w, h = 2178, 2, 33, 33
    ...
    def forward(self, x):
        n, c, h, w = x.detach().size() # torch.Size([200, 2, 33, 33])
        
        encode1 = self.encoder1(x) # same size as x
        encode2 = self.encoder2(x) # same size as x
...
class CLNet(nn.Module):
    ...
    def forward(self, x):
        ...
        encode1 = self.encoder1(x) # same size as x
        encode1 = self.dropout(encode1)
        encode1 = self.sa(encode1)  # torch.Size([1, 2, 33, 33])
        encode2 = self.encoder2(x) # same size as x
        encode2 = self.se(encode2)
        out = torch.cat((encode1, encode2), dim=1) #torch.Size([1, 34, 33, 33])
        out = self.encoder_conv(out)
        out = self.dropout(out)
...
class CSINet(nn.Module):
    ...
    def forward(self, x):
        ...
        encode1 = self.encoder1(x) # same size as x
        encode1 = self.dropout(encode1)
        out = self.encoder_fc(encode1.view(n, -1))
        
        out = self.decoder_fc(out).view(n, c, h, w) #torch.Size([1, 2, 33, 33])
        # two refineNet
        out = self.decoder_feature(out)
